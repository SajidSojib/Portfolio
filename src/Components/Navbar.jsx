/* eslint-disable no-unused-vars */
import React, { useContext, useEffect, useState } from "react";
import { Spin as Hamburger } from "hamburger-react";
import { motion } from "motion/react";
import me from '../assets/me.png'
import { toast } from "react-toastify";
import { Link } from "react-scroll";

const Navbar = () => {

  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [theme, setTheme] = useState(() => {
    return localStorage.getItem("theme") || "dark";
  });

  // const isLoggedIn = localStorage.getItem("login") === "true";

  // if (isLoggedIn) {
  //   console.log("User is logged in");
  // }


  useEffect(() => {
    document.body.setAttribute("data-theme", theme);
    localStorage.setItem("theme", theme);
  }, [theme]);

  const toggleTheme = () => {
    setTheme(theme === "light" ? "dark" : "light");
  };

 

  const links = (
    <>
      <li
        data-aos="fade-down"
        data-aos-duration="400"
        data-aos-easing="ease-in-out"
        data-aos-once="true"
      >
        <Link
          to="banner"
          smooth={true}
          offset={-64}
          duration={500}
          spy={true}
          exact="true"
          activeClass="after:w-full"
          className="relative p-2 block lg:p-0 text-primary font-medium tracking-wide transition-colors duration-300
          after:content-[''] after:absolute after:left-1/2 after:bottom-0 after:h-[2px] after:bg-primary after:w-0 after:transition-all after:duration-300 after:-translate-x-1/2
          hover:after:w-full"
          aria-label="Home"
          title="Home"
        >
          Home
        </Link>
      </li>
      <li
        data-aos="fade-down"
        data-aos-duration="600"
        data-aos-easing="ease-in-out"
        data-aos-once="true"
      >
        <Link
          to="about"
          smooth={true}
          offset={-64}
          duration={500}
          spy={true}
          exact="true"
          activeClass="after:w-full"
          className="relative p-2 block lg:p-0 font-medium tracking-wide transition-colors duration-300
          after:content-[''] after:absolute after:left-1/2 after:bottom-0 after:h-[2px] after:bg-primary after:w-0 after:transition-all after:duration-300 after:-translate-x-1/2
          hover:after:w-full"
          aria-label="About"
          title="About"
        >
          About
        </Link>
      </li>
      <li
        data-aos="fade-down"
        data-aos-duration="800"
        data-aos-easing="ease-in-out"
        data-aos-once="true"
      >
        <Link
          to="skills"
          smooth={true}
          offset={-80}
          duration={500}
          spy={true}
          exact="true"
          activeClass="after:w-full"
          className="relative p-2 block lg:p-0 font-medium tracking-wide transition-colors duration-300
          after:content-[''] after:absolute after:left-1/2 after:bottom-0 after:h-[2px] after:bg-primary after:w-0 after:transition-all after:duration-300 after:-translate-x-1/2
          hover:after:w-full"
          aria-label="Skills"
          title="Skills"
        >
          Skills
        </Link>
      </li>
      <li
        data-aos="fade-down"
        data-aos-duration="1000"
        data-aos-easing="ease-in-out"
        data-aos-once="true"
      >
        <Link
          to="projects"
          smooth={true}
          offset={-64}
          duration={500}
          spy={true}
          exact="true"
          activeClass="after:w-full"
          className="relative p-2 block lg:p-0 font-medium tracking-wide transition-colors duration-300
          after:content-[''] after:absolute after:left-1/2 after:bottom-0 after:h-[2px] after:bg-primary after:w-0 after:transition-all after:duration-300 after:-translate-x-1/2
          hover:after:w-full"
          aria-label="Projects"
          title="Projects"
        >
          Projects
        </Link>
      </li>
      <li
        data-aos="fade-down"
        data-aos-duration="1200"
        data-aos-easing="ease-in-out"
        data-aos-once="true"
      >
        <Link
          to="contact"
          smooth={true}
          offset={-64}
          duration={500}
          spy={true}
          exact="true"
          activeClass="after:w-full"
          className="relative p-2 block lg:p-0 font-medium tracking-wide transition-colors duration-300
          after:content-[''] after:absolute after:left-1/2 after:bottom-0 after:h-[2px] after:bg-primary after:w-0 after:transition-all after:duration-300 after:-translate-x-1/2
          hover:after:w-full"
          aria-label="Contact"
          title="Contact"
        >
          Contact
        </Link>
      </li>
    </>
  );

  const link2 = (
    <>
      {/* theme btn */}
      {/* <li className="hidden lg:block">
        <label className="swap swap-rotate">
          <input
            onChange={toggleTheme}
            checked={theme === "dark" ? true : false}
            type="checkbox"
            className="theme-controller"
            value="synthwave"
          />

          <svg
            className={`swap-off h-10 w-10 fill-current`}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
          >
            <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
          </svg>

          <svg
            className={`swap-on h-10 w-10 fill-current`}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
          >
            <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
          </svg>
        </label>
      </li> */}

      <li>
        <button
          className="hidden lg:flex btn btn-primary text-base-300"
          onClick={()=>toast.warning("Under Development")}
        >
          Resume
        </button>
      </li>
    </>
  );

  return (
    <div className="px-4 text-primary py-3 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 ">
      <div className="relative flex items-center justify-between">
        <Link
          to={"/"}
          aria-label="TrueReview"
          title="Company"
          className="inline-flex items-center"
        >
          <span className="text-xl font-bold text-white tracking-wide">
            <span className="text-primary">{"<"}</span>Sajid{" "}
            <span className="text-primary">{"/>"}</span>
          </span>
        </Link>
        <ul className="items-center hidden space-x-6 lg:flex">
          {links}
          {link2}
        </ul>

        {/* phone */}
        <div className="relative lg:hidden">
          <div className="flex text-primary items-center">
            {/* <div className="text-primary">
              <label className="swap swap-rotate">
                <input
                  onChange={toggleTheme}
                  checked={theme === "dark" ? true : false}
                  type="checkbox"
                  className="theme-controller"
                  value="synthwave"
                />

                <svg
                  className={`swap-off h-8 w-8 fill-current`}
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                >
                  <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
                </svg>

                <svg
                  className={`swap-on h-8 w-8 fill-current`}
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                >
                  <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
                </svg>
              </label>
            </div> */}
            <Hamburger
              toggled={isMenuOpen}
              toggle={setIsMenuOpen}
              size={25}
            ></Hamburger>
          </div>

          {isMenuOpen && (
            <motion.div
              animate={{
                x: [100, 0],
                transition: { duration: 0.3 },
                opacity: 1,
              }}
              initial={{ opacity: 0 }}
              className="absolute right-0 z-40 overflow-y-visible top-10"
            >
              <div className="h-full p-3 space-y-2 w-60 bg-[#0b4249] rounded-md">
                <div className="flex items-center space-x-2">
                  <img
                    className="w-10 h-10 rounded-full ring-2 ring-offset-4 bg-accent ring-primary dark:ring-offset-gray-100"
                    src={me}
                    alt="Bordered avatar"
                  />

                  <div>
                    <h2 className="text-lg font-semibold">Sajid Ahmed Sojib</h2>
                    <span className="flex items-center space-x-1">
                      <p className="text-xs hover:underline text-gray-300">
                        sajidahmed2401@gmail.com
                      </p>
                    </span>
                  </div>
                </div>
                <div className="divide-y dark:divide-gray-300">
                  <ul className="pt-2 pb-4 space-y-1 text-sm ">{links}</ul>
                </div>
              </div>
            </motion.div>
          )}
        </div>
      </div>
    </div>
  );
};

export default Navbar;
